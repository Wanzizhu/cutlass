...
template <typename T> void dump_tensor(T *tensor, int m, int n, int ld) {
  printf("dumping tensor data\n");
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
      printf("%.4f ", tensor[i + j * ld]);
    }
    printf("\n");
  }
}
...
bool all_close(const T1 *actual, int stride_m0, int stride_n0,
               const T2 *desired, int stride_m1, int stride_n1, const int M,
               const int N, const float rtol = 1e-3f,
  ...
  for (int m = 0; m < M; ++m) {
    for (int n = 0; n < N; ++n) {
      const float a = static_cast<float>(actual[m * stride_m0 + n * stride_n0]);
      const float b =
          static_cast<float>(desired[m * stride_m1 + n * stride_n1]);
  ...
  if (maximum_err > tol) {
    ...
    printf("Error! Matrix[%d, %d]=%.8f, ref=%.8f, error = %.8f, error term is "
           "> %E\n",
           m, n, float(actual[m * stride_m0 + n * stride_n0]),
           float(desired[m * stride_m1 + n * stride_n1]), maximum_err, tol);
    fflush(stdout);
    return false;
  }
  return true;
...
template <class ProblemShape, class CtaTiler, class TA, class AStride,
          ...
          class LocalAccA, class LocalAccB>
__global__ static void
gemm_device(ProblemShape shape_MNK, CtaTiler cta_tiler, TA const *A, AStride dA,
            ASmemLayout sA_layout, TiledCopyA copy_a, TB const *B, BStride dB,
            BSmemLayout sB_layout, TiledCopyB copy_b, TC *C, CStride dC,
            CSmemLayout, TiledMma mma, Alpha alpha, Beta beta,
            sycl::nd_item<2> &item, LocalAccA &smemAcc, LocalAccB &smemBcc) {
  using namespace cute;

  auto blockIdxX = item.get_group(0);
  auto blockIdxY = item.get_group(1);
  auto threadIdxX = item.get_local_linear_id();
  ...
  TA *smemA =
      smemAcc.template get_multi_ptr<sycl::access::decorated::no>().get();
  TB *smemB =
      smemBcc.template get_multi_ptr<sycl::access::decorated::no>().get();
  ...
  for (int k_tile = 0; k_tile < K_TILE_MAX; ++k_tile) {
    ...
    sycl::group_barrier(
        item.get_group()); // Wait for all threads to consume smem
    copy(tArA, tAsA);
    copy(tBrB, tBsB);
    sycl::group_barrier(
        item.get_group()); // Wait for all threads to consume smem
    ...
    // TUTORIAL: The above call to copy(copy_a, tAgA(_,_,_,k_tile_next), tArA)
    // is equivalent to
    //   CUTE_UNROLL
...
sycl::event gemm_nt(int m, int n, int k, Alpha alpha, TA const *A, int ldA,
  ...
  // thread. Reproduce that atom 16x16x1 times (m-major) across threads so that
  // we use 256 threads.
  ...
  sycl::range<2> dimBlock(1, size(mmaC));
  sycl::range<2> dimGrid(size(ceil_div(M, bM)), size(ceil_div(N, bN)));
  auto cgh = [=](sycl::handler &cgh) {
    sycl::local_accessor<TA, 1> smemA(sycl::range<1>{sA_size}, cgh);
    sycl::local_accessor<TB, 1> smemB(sycl::range<1>{sB_size}, cgh);
    cgh.parallel_for(sycl::nd_range<2>(dimGrid * dimBlock, dimBlock),
                     [=](sycl::nd_item<2> item) {
                       gemm_device(prob_shape, cta_tiler, A, dA, sA, copyA, B,
                                   dB, sB, copyB, C, dC, sC, mmaC, alpha, beta,
                                   item, smemA, smemB);
                     });
  };
  auto event = queue.submit(cgh);
  return event;
}

// Setup params for a TN GEMM
template <class TA, class TB, class TC, class Alpha, class Beta>
sycl::event gemm_tn(int m, int n, int k, Alpha alpha, TA const *A, int ldA,
                    TB const *B, int ldB, Beta beta, TC *C, int ldC,
                    sycl::queue &queue) {
  using namespace cute;
  ...
  auto cgh = [=](sycl::handler &cgh) {
    ...
    sycl::local_accessor<TB, 1> smemB(sycl::range<1>{sB_size}, cgh);
    cgh.parallel_for(
        sycl::nd_range<2>(dimGrid, dimBlock), [=](sycl::nd_item<2> item) {
          gemm_device(prob_shape, cta_tiler, A, dA, sA, copyA, B, dB, sB, copyB,
                      C, dC, sC, mmaC, alpha, beta, item, smemA, smemB);
        });
  };
  auto event = queue.submit(cgh);
  return event;
...
template <class TA, class TB, class TC, class Alpha, class Beta>
sycl::event gemm(char transA, char transB, int m, int n, int k, Alpha alpha,
                 TA const *A, int ldA, TB const *B, int ldB, Beta beta, TC *C,
                 int ldC, sycl::queue &stream) {
...
int main(int argc, char **argv) {
  ...
  TA *d_A = sycl::malloc_device<TA>(m * k, queue);
  TB *d_B = sycl::malloc_device<TB>(n * k, queue);
  TC *d_C = sycl::malloc_device<TC>(m * n, queue);

  queue.memcpy(d_A, h_A.data(), m * k * sizeof(TA)).wait();
  queue.memcpy(d_B, h_B.data(), n * k * sizeof(TB)).wait();
  queue.memcpy(d_C, h_C.data(), m * n * sizeof(TC)).wait();
  ...
  queue.wait();
  queue.memcpy(h_C.data(), d_C, m * n * sizeof(TC)).wait();

  std::vector<float> expected_result(m * n, 0);
  auto trans_a_blas = transA == 'T' ? CblasTrans : CblasNoTrans;
  auto trans_b_blas = transB == 'T' ? CblasTrans : CblasNoTrans;
  cblas_sgemm(CblasColMajor, trans_a_blas, trans_b_blas, m, n, k, 1.0f,
              h_A.data(), ldA, h_B.data(), ldB, 0, expected_result.data(), ldC);

  dump_tensor(h_C.data(), 1, 16, ldC);

  bool passed =
      all_close(h_C.data(), 1, ldC, expected_result.data(), 1, ldC, m, n);
  ...
  for (int i = 0; i < timing_iterations; ++i) {
    auto event = gemm(transA, transB, m, n, k, alpha, d_A, ldA, d_B, ldB, beta,
                      d_C, ldC, queue);
  ...
  double gflops = ops / 1e6 / cute_time;
  printf("CUTE_GEMM:     %.3f GFlop/s  %6.4f us\n", gflops / cute_time,
         cute_time * 1e3);
